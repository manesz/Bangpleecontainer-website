...
class ContactData
{
    ...
    public function contactDataPost($post)
    {
        $contactDataID = $post['contactDataID'];
        $latitude = $post['latitude'];
        $longitude = $post['longitude'];
        $arrayImage = $post['pathImg'];
        $arrayLabel = $post['label'];
        $arrayValue = $post['value'];
        if (intval($contactDataID) == 0) {
            $contactDataID = $this->insertContactData($latitude, $longitude);
        } else {
            $this->deleteMapContactData($contactDataID);
            $this->updateContactData($contactDataID, $latitude, $longitude);
        }
        foreach ($arrayLabel as $key => $value) {
            $this->insertMapContactData($contactDataID, $value, $arrayValue[$key]);
        }
        foreach ($arrayImage as $key => $value) {
            $this->insertMapContactDataImage($contactDataID, $value);
        }
        return 'finish';
    }
    ...
    public function getImageFromContactDataID($contactDataID)
    {
        $strSql = "
                SELECT
                  i.*
                FROM
                  map_contact_data_image c
                  INNER JOIN image i
                    ON (c.image_id = i.id)
                WHERE c.public = 1
                  AND i.public = 1
                  AND c.contact_data_id = $contactDataID
                ";
    ...
    public function getDataFromContactDataID($contactDataID)
    {
        $strSql = "
            SELECT
              *
            FROM
              map_contact_data
            WHERE public = 1
              AND contact_data_id = $contactDataID
        ";
        $myrows = $this->wpdb->get_results($strSql);
        return $myrows;
    }
    ...
    public function insertContactData($lat, $long)
    {
        $this->wpdb->insert(
            'contact_data',
            array(
    ...
    public function updateContactData($contactID, $lat, $long)
    {
        $this->wpdb->update(
            'contact_data',
            array(
    ...
    public function insertMapContactData($contactDataID, $label, $value)
    {
        $this->wpdb->insert(
            'map_contact_data',
            array(
    ...
    public function insertMapContactDataImage($contactDataID, $pathImg, $title = '')
    {
        $imageID = addImage($pathImg, $title);
        $this->wpdb->insert(
            'map_contact_data_image',
            array(
                'image_id' => $imageID,
                'contact_data_id' => $contactDataID,
                'public' => 1
    ...
    /*
     * ลบข้อมูลทีเดียว 3 ตาราง
     */
    public function deleteMapContactData($contactDataID)
    {
        $strSql = "
            DELETE
              c,
            ...
            FROM
              ...
              INNER JOIN `image` i
                ON (c.`image_id` = i.`id`)
              INNER JOIN `map_contact_data` d
                ON (
                  d.`contact_data_id` = c.`contact_data_id`
                )
            WHERE c.`contact_data_id` = $contactDataID
        ";

...
require_once ("class/ClassSlide.php");
$conDB = new Slide($wpdb);
$checkPost = @$_POST['slideCheckPost'];
switch ($checkPost) {
    case "add":
        $slideID = $conDB->addSlide(@$_POST['pathImg'], @$_POST['titleSlide'], @$_POST['description'], @$_POST['slideLink']);
        if ($slideID != false) {
            echo $slideID;
        } else {
            echo 'false';
        ...
        break;
    case "update":
        $slideID = $conDB->updateSlideByID(@$_POST['pathImg'], @$_POST['titleSlide'], @$_POST['description'], @$_POST['slideLink'], @$_POST['slideID']);
        if ($slideID != false) {
            echo $slideID;
        } else {
            echo 'false';
        ...
        break;
    case "delete":
        $result = $conDB->updateSlide(@$_POST['slideID'], 0);
        if ($result != false) {
            echo 'finish';
        } else {
            echo 'false';
        ...
        break;
    case "get":
        $result = $conDB->loadListSlide(@$_POST['slideID']);
        if ($result != false) {
            ?>
        ...
        <table class="wp-list-table widefat" cellspacing="0">
            <thead>
            <tr>
                <th scope="col" class="manage-column column-name" style=""><strong><font
                    color="#F9651B">Name</font></strong></th>
                <th scope="col" class="manage-column column-name" style=""><strong><font
                    color="#F9651B">Value</font></strong></th>
            </tr>
            </thead>
            <tbody id="the-list">
            <tr class="alternate">
                <td>
                    <input type="text" id="slideTitle" size="50" placeholder="Title"
                           value="<?php echo $result[0]->title; ?>"/>
                </td>
                <td>
                    <textarea rows="5" cols="40" id="slideDescription" placeholder="Description"
                              style="margin: 1px; width: 391px; height: 88px;"><?php echo $result[0]->description; ?></textarea>
                </td>
            </tr>
            <tr scope="row" class="alternate">
                <td>&nbsp;</td>
                <td>
                    <textarea rows="5" cols="40" id="slideLink" placeholder="Link Slide"
                              style="margin: 1px; width: 391px; height: 88px;"><?php echo $result[0]->link; ?></textarea>
...
<script type="text/javascript">
    ...
    var slideDescription = $jConflict('#slideDescription').val();
    var slideLink = $jConflict('#slideLink').val();
    $jConflict(document).ready(function () {
        ...
        $jConflict("#slideTitle").focus();

        $jConflict('#slideDescription').focus(
            function () {
                if ($jConflict(this).val() == slideDescription)
                    $jConflict(this).val("");
            }
        );
        $jConflict('#slideDescription').blur(
            function () {
                if ($jConflict(this).val() == "")
                    $jConflict(this).val(slideDescription);
            }
        );
        $jConflict('#slideLink').focus(
            function () {
                if ($jConflict(this).val() == slideLink)
                    $jConflict(this).val("");
            }
        );
        $jConflict('#slideLink').blur(
            function () {
                if ($jConflict(this).val() == "")
                    $jConflict(this).val(slideLink);
            }
        );
        $jConflict("#slide-post").submit(function () {
            if (!validate())
                return false;
            $jConflict.ajax({
                type:"POST",
                url:urlPost,
                data:{
                    pathImg:this.pathImg.value,
                    titleSlide:this.slideTitle.value,
                    description:this.slideDescription.value,
                    slideLink:this.slideLink.value,
                    slideCheckPost:this.typePost.value
        ...
        $jConflict(".imgHover").mouseover(function () {
            var id = this.id.split("_");
            idHover = id[1];
            $jConflict("#btnDelete_" + idHover).show();
        }).mouseout(function () {
                $jConflict("#btnDelete_" + idHover).hide();
            });
    ...
    function validate()
    {
        if ($jConflict('#slideTitle').val() == ""){
            alert("กรุณากรอก Title");
            $jConflict('#slideTitle').focus();
            return false;
        }
        if ($jConflict('#pathImg').val() == ""){
            alert("กรุณาเลือกรูปภาพ");
            $jConflict('#pathImg').focus();
            return false;
        }
        return true;
    }
...
<?php
$listSlide = $conDB->loadListSlide();
if (!empty($listSlide)) {
    ?>
<div id="container">
    <ul id="items">
        <?php
        foreach ($listSlide as $key => $value) { //getIDImageFromUrl($value->path);
            ?>
            <li style="display: list-item;">
                <div class="imgHover" id="imgIDHover_<?php echo $value->id; ?>">
                    <p class="image" id="imgShow_<?php echo $value->id; ?>"><img width="100" height="60"
                    ...
                    <div class="divDelete" id="btnDelete_<?php echo $value->id; ?>">
                        <img class="deleteImage" id="deleteImage_<?php echo $value->id; ?>"
...
<form action="" id="slide-post" method="post">
    <div id="div-inner">
        <input type="hidden" id="typePost" value="add"/>
        <table class="wp-list-table widefat" cellspacing="0">
            <thead>
            <tr>
                <th scope="col" class="manage-column column-name" style=""><strong><font
            ...
            <tr class="alternate">
                ...
                <td>
                    <textarea rows="5" cols="40" id="slideDescription" placeholder="Description"
                              style="margin: 1px; width: 391px; height: 88px;">Description</textarea>
                </td>
            </tr>
            <tr scope="row" class="alternate">
                <td>&nbsp;</td>
                <td>
                    <textarea rows="5" cols="40" id="slideLink" placeholder="Link Slide"
                              style="margin: 1px; width: 391px; height: 88px;">Link Slide</textarea>

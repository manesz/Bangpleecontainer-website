...
require_once ("class/ClassContactData.php");
$conDB = new ContactData($wpdb);
$checkPost = @$_POST['typePost'];
switch ($checkPost) {
    case "post":
        $result = $conDB->contactDataPost(@$_POST);
        if ($result != false) {
            echo $result;
        } else {
            echo 'false';
...
<?php
$getContactData = $conDB->getContactData();
$latitude = $getContactData[0]->latitude;
$longitude = $getContactData[0]->longitude;
$contactDataID = $getContactData[0]->id;
$arrayImage = $conDB->getImageFromContactDataID($contactDataID);
$arrayData = $conDB->getDataFromContactDataID($contactDataID);
?>
<link href="<?php bloginfo('template_directory');?>/theme-option-controller/css/easypaginate.css" rel="stylesheet"
      type="text/css"/>
<link href="<?php bloginfo('template_directory');?>/theme-option-controller/css/style.css" rel="stylesheet"
      type="text/css"/>
<!--<script type="text/javascript"-->
<!--        src="--><?php //bloginfo('template_directory');?><!--/theme-option-controller/js/jquery.min.js"></script>-->
<script type="text/javascript"
        src="<?php bloginfo('template_directory');?>/theme-option-controller/js/jquery-1.8.2.min.js"></script>
<script type="text/javascript"
        src="<?php bloginfo('template_directory');?>/theme-option-controller/js/easypaginate.js"></script>
<!--fancybox-->
<!--<script type="text/javascript" src="--><?php //bloginfo('template_directory');?><!--/theme-option-controller/js/fancybox.js"></script>-->

<script type="text/javascript">
var $jConflict = jQuery.noConflict();
var urlTab = "tab5";
var urlPost = "<?php echo network_site_url('/');?>?tabPage=" + urlTab;
$jConflict(document).ready(function () {
    ...
    $jConflict("#contact-data").submit(function () {
        $jConflict.ajax({
            type:"POST",
            url:urlPost,
            data:$jConflict("#contact-data").serialize(),
    ...
    var idImageHover;
    $jConflict('.imageHover').live('mouseover',function () {
        var id = this.id.split("_");
        idImageHover = id[1];
        showImg("#btnDeleteImage_" + idImageHover);
    }).live('mouseout', function () {
            hideImg("#btnDeleteImage_" + idImageHover);
        });

    var idDataHover;
    $jConflict('.dataHover').live('mouseover',function () {
        var id = this.id.split("_");
        idDataHover = id[1];
        showImg("#btnDeleteData_" + idDataHover);
    }).live('mouseout', function () {
            hideImg("#btnDeleteData_" + idDataHover);
        });
});

var countValueSave = 1;
var arrayImage = [
<?php
if (!empty($arrayImage)) {
...
function readValueImageFromInput() {
    if (arrayImage.length > 0) {
        for (i = 0; i < arrayImage.length; i++) {
            var id = i + 1;
            id = "#pathImg_" + id;
            arrayImage[i][0] = $jConflict(id).val();
        }
    }
}
function readValueDataFromInput() {
    if (arrayData.length > 0) {
        for (i = 0; i < arrayData.length; i++) {
            var id = i + 1;
            arrayData[i][0] = $jConflict("#label_" + id).val();
            arrayData[i][1] = $jConflict("#value_" + id).val();
        }
    }
}

function refreshArrayImage(id) {
    var count = 0;
    var newStr = '';
    var newArray = [];
    for (i = 0; i < arrayImage.length; i++) {
        if (i + 1 != (id)) {
            newArray.push(arrayImage[i]);
            count++;
            newStr += getStrImageHtml(count, arrayImage[i][0]);
        }
    }
    arrayImage = newArray;
    $jConflict("#td-list-image").html(newStr);
}

function refreshArrayData(id) {
    var count = 0;
    var newStr = '';
    var newArray = [];
    for (i = 0; i < arrayData.length; i++) {
        if (i + 1 != id) {
            ...
            count++;
            newStr += getStrDataHtml(count, arrayData[i][0], arrayData[i][1]);
        }
    }
    arrayData = newArray;
    $jConflict("#td-list-data").html(newStr);
}

function getStrImageHtml(id, value) {
    if (id == 1) {
        var strHtml = '<div>' +
            '<input id="pathImg_1" type="text" name="pathImg[]" size="100" placeholder="Path Image ' + id + '"' +
            'title="Path Image 1" value="' + value + '"/>&nbsp;' +
            '<input id="uploadImageButton" type="button" class="button" value="Upload Image"' +
            'onclick="imageUploader(' + "'#pathImg_1'" + ');"/>&nbsp;' +
            '<img class="addValue" onclick="clickAddImage()"' +
            'src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-add.png"' +
            'title="เพิ่ม Image"/>' +
            '</div>';
    } else {
        var strHtml =
            '<div class="imageHover" id="imageHover_' + id + '">' +
                '<input id="pathImg_' + id + '" type="text" name="pathImg[]" size="100"' +
                'placeholder="Path Image ' + id + '"' +
                'title="Path Image" value="' + value + '"/>&nbsp;' +
                '<input id="uploadImageButton" type="button" class="button" value="Upload Image"' +
                'onclick="imageUploader(' + "'#pathImg_" + id + "'" + ');"/>&nbsp;' +
                '<img class="deleteValue" id="btnDeleteImage_' + id + '" onclick="clickDeleteImage(this)"' +
                'src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-delete.png"' +
                'title="ลบ Image"/></div>';
    }
    return strHtml;
}

function getStrDataHtml(id, label, value) {
    if (id == 1) {
        var strHtml =
            '<div>' +
                '<label><strong><font ' +
                'color="#F9651B">Label 1</font></strong>&nbsp;&nbsp;&nbsp;' +
                '<input id="label_1" type="text" value="' + label + '" placeholder="Label 1"' +
                'name="label[]" size="30"/></label>' +
                '<img class="addValue" style="float: right;" onclick="clickAddData()"' +
                'src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-add.png"' +
                'title="เพิ่ม Value"/><br>' +
                '<label><strong><font ' +
                'color="#F9651B">Value 1</font></strong>&nbsp;&nbsp;' +
                '<input id="value_1" type="text" value="' + value + '" name="value[]"' +
                'size="100"/ placeholder="Value 1"></label>' +
                '</div><br>';
    } else {
        var strHtml =
            '<div class="dataHover" id="dataHover_' + id + '">' +
                '<label><strong><font ' +
                'color="#F9651B">Label ' + id + '</font></strong>&nbsp;&nbsp;&nbsp;' +
                '<input id="label_' + id + '" type="text"' +
                'value="' + label + '" placeholder="Label ' + id + '"' +
                'name="label[]" size="30"/></label>' +
                '<img class="deleteValue" style="float: right;" onclick="clickDeleteData(this)"' +
                'src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-delete.png"' +
                'title="ลบ Value" id="btnDeleteData_' + id + '"/><br>' +
                '<label><strong><font ' +
                'color="#F9651B">Value ' + id + '</font></strong>&nbsp;&nbsp;' +
                '<input id="value_' + id + '" type="text" name="value[]"' +
                'size="100" value="' + value + '" placeholder="Value ' + id + '"/></label>' +
                '</div><br>';
    }
    return strHtml;
}
...
function clickAddImage() {
    arrayImage.push(['']);
    var strHtml = getStrImageHtml(arrayImage.length, '');
    $jConflict("#td-list-image").append(strHtml);
}

function clickDeleteImage(event) {
    readValueImageFromInput();
    var id = event.id.split("_");
    id = id[1];
    refreshArrayImage(id);
}

function clickAddData() {
    arrayData.push([ '', '']);
    var strHtml = getStrDataHtml(arrayData.length, '', '');
    $jConflict("#td-list-data").append(strHtml);
}

function clickDeleteData(event) {
    readValueDataFromInput();
    var id = event.id.split("_");
    id = id[1];
    refreshArrayData(id);
}
...
<div id="container">
    <img src="<?php if ($arrayImage) echo $arrayImage[0]->path; else echo ''; ?>" width="450" height="180" alt=""/>
</div>
<!--<h3>Upload Slide</h3>-->
<form action="" id="contact-data" method="post">
    <input type="hidden" id="typePost" name="typePost" value="post"/>
    <input type="hidden" id="contactDataID" name="contactDataID"
           value="<?php if ($contactDataID) echo $contactDataID; else echo 0; ?>"/>
    <table class="wp-list-table widefat" cellspacing="0">
        <tr>
            <th scope="col" class="manage-column column-name" colspan="2"><label for="pathImg_1"><strong><font
                color="#F9651B">รูปภาพแผนที่</font></strong></label></th>
        </tr>
        <tr>
            <td colspan="2" id="td-list-image">
                <?php
                if (!empty($arrayImage)) {
                    foreach ($arrayImage as $key => $value) {
                        if ($key == 0) {
                            ?>
                            <div>
                                <input id="pathImg_1" type="text" name="pathImg[]" size="100" placeholder="Path Image 1"
                                       title="Path Image 1" value="<?php echo $value->path; ?>"/>
                                <input id="uploadImageButton" type="button" class="button" value="Upload Image"
                                       onclick="imageUploader('#pathImg_1');"/>
                                <img class="addValue" onclick="clickAddImage()"
                                     src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-add.png"
                            ...
                            ?>
                            <div class="imageHover" id="imageHover_<?php echo $key + 1; ?>">
                                <input id="pathImg_<?php echo $key + 1; ?>" type="text" name="pathImg[]" size="100"
                                       placeholder="Path Image <?php echo $key + 1; ?>"
                                       title="Path Image" value="<?php echo $value->path; ?>"/>
                                <input id="uploadImageButton" type="button" class="button" value="Upload Image"
                                       onclick="imageUploader('#pathImg_<?php echo $key + 1; ?>');"/>
                                <img class="deleteValue" id="btnDeleteImage_<?php echo $key + 1; ?>"
        ...
        <tr>
            <td class="column-title">
                <label><strong><font
                    color="#F9651B">Latitude</font></strong>&nbsp;
                    <input id="latitude" type="text" name="latitude"
                           size="30" placeholder="Latitude"
                           title="Latitude" value="<?php echo $latitude; ?>"/></label>
            </td>
            <td>
                <label><strong><font
                    color="#F9651B">Longitude</font></strong>&nbsp;
                    <input id="longitude" type="text" name="longitude"
                           size="30" placeholder="Longitude"
                           title="Longitude" value="<?php echo $longitude; ?>"/></label>
            </td>
        </tr>
        <tr>
            <td colspan="2" id="td-list-data">
                <?php
                if (!empty($arrayData)) {
                    foreach ($arrayData as $key => $value) {
                        if ($key == 0) {
                            ...
                            <div>
                                <label><strong><font
                                    color="#F9651B">Label <?php echo $key + 1; ?></font></strong>&nbsp;&nbsp;&nbsp;
                                    <input id="label_<?php echo $key + 1; ?>" type="text"
                                           value="<?php echo $value->label;?>" placeholder="Label 1"
                                           name="label[]" size="30"/></label>
                                <img class="addValue" style="float: right;" onclick="clickAddData()"
                                     src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-add.png"
                                     title="เพิ่ม Value"/><br>
                                <label><strong><font
                                    color="#F9651B">Value <?php echo $key + 1; ?></font></strong>&nbsp;&nbsp;
                                    <input id="value_<?php echo $key + 1; ?>" type="text" name="value[]"
                                           size="100" value="<?php echo $value->value;?>"
                                           placeholder="Value 1"/></label>
                            </div><br>
                            <?php
                        } else {
                            ?>
                            <div class="dataHover" id="dataHover_<?php echo $key + 1; ?>">
                                <label><strong><font
                                    color="#F9651B">Label <?php echo $key + 1; ?></font></strong>&nbsp;&nbsp;&nbsp;
                                    <input id="label_<?php echo $key + 1; ?>" type="text"
                                ...
                                <img class="deleteValue" style="float: right;" onclick="clickDeleteData(this)"
                                     src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-delete.png"
                                     title="ลบ Value" id="btnDeleteData_<?php echo $key + 1; ?>"/><br>
                                <label><strong><font
                                    color="#F9651B">Value <?php echo $key + 1; ?></font></strong>&nbsp;&nbsp;
                                    <input id="value_<?php echo $key + 1; ?>" type="text" name="value[]"
                                           size="100" value="<?php echo $value->value;?>"
                                           placeholder="Value <?php echo $key + 1; ?>"/></label>
                            </div><br>
                            <?php
                        }
                    }
                    ...
                    <div>
                        <label><strong><font
                            color="#F9651B">Label 1</font></strong>&nbsp;&nbsp;&nbsp;
                            <input id="label_1" type="text" placeholder="Label 1"
                                   name="label[]" size="30"/></label>
                        <img class="addValue" style="float: right;" onclick="clickAddData()"
                             src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-add.png"
                             title="เพิ่ม Value"/><br>
                        <label><strong><font
                            color="#F9651B">Value 1</font></strong>&nbsp;&nbsp;
                            <input id="value_1" type="text" name="value[]"
                                   size="100" placeholder="Value 1"/></label>
                    </div><br>
                    <?php
                }?>
        ...
        <tr align="right">
            <td colspan="2">
                <input type="submit" name="priceTable" value="Save" class="button-primary"/>
            </td>
        </tr>
    </table>
</form>

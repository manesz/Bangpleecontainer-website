...
$conDB = new ImageGallery($wpdb);
$checkPost = @$_POST['galleryCheckPost'];

switch ($checkPost) {
    case "save":
        $imageID = $conDB->addImageGallery(@$_POST['pathImg'], @$_POST['titleImg']);
        if ($imageID != false) {
            echo $imageID;
        } else {
            echo 'false';
        ...
        break;
    case "delete":
        $result = $conDB->updateGallery(@$_POST['galleryID'], 0);
        if ($result != false) {
            echo 'finish';
        } else {
            echo 'false';
...
<script type="text/javascript">
    ...
    var urlPost = "<?php echo network_site_url('/');?>?tabPage=tab1";
    $jConflict(document).ready(function () {
        ...
        $jConflict("#gallery-post").submit(function () {
            $jConflict.ajax({
                type:"POST",
                url:urlPost,
                data:{
...
<?php
$listImage = $conDB->loadListImage();
if (!empty($listImage)) {
    ?>
<div id="container">
    <ul id="items">
        <?php
        foreach ($listImage as $key => $value) { //getIDImageFromUrl($value->path);
            ?>
            <li style="display: list-item;">
                <div class="imgHover" id="imgIDHover_<?php echo $value->g_id; ?>">
                    <p class="image"><img width="100" height="60"
                                          src="<?php echo $value->path; ?>" alt=""
                                          title="<?php echo $value->i_title; ?>"></p>
                    <!--<h3>Template --><?php //echo $value->g_id; ?><!--</h3>-->
                    <p class="info"></p>

                    <div class="divDelete" id="btnDelete_<?php echo $value->g_id; ?>">
                        <img class="deleteImage" id="deleteImage_<?php echo $value->g_id; ?>"
...
<h3><label for="pathImg">Upload รูปภาพ</label></h3>
<form action="" id="gallery-post" method="post">
    <input id="pathImg" type="text" name="pathImg" size="50" placeholder="Path Image"/>

...
require_once ("class/ClassVideoGallery.php");
$conDB = new VideoGallery($wpdb);
$checkPost = @$_POST['typePost'];

switch ($checkPost) {
    case "add":
        $videoID = $conDB->addVideo(@$_POST['videoScript'], @$_POST['videoTitle']);
        if ($videoID != false) {
            echo $videoID;
        } else {
            echo 'false';
        ...
        break;
    case "delete":
        $result = $conDB->updatePublicVideo(@$_POST['videoID'], 0);
        if ($result != false) {
            echo 'finish';
        } else {
            echo 'false';
        ...
        break;
    case "get":
        $result = $conDB->loadListVideo(@$_POST['videoID']);
        if ($result != false) {
            ?>
        <th scope="row" class="check-column">
            <td>
                <input type="hidden" id="typePost" name="typePost" value="update"/>
                <input type="hidden" name="videoID" value="<?php echo $result[0]->id; ?>"/>
                <textarea rows="5" cols="40" id="videoScript" name="videoScript"
                          placeholder="Video Script" style="margin: 1px; width: 692px;
                    height: 89px;"><?php echo str_replace("\\", "", $result[0]->script); ?></textarea>
                <br>
                <input type="button" onclick="document.location.reload(true)" value="New" class="button-primary"/>
                <input type="submit" name="submit" value="Update" class="button-primary"/></td>
            </td>
        </th>
        <?php
        }
        ...
        break;
    case "update":
        $result = $conDB->updateVideo(@$_POST['videoID'], @$_POST['videoScript']);
        if ($result != false) {
            echo 'finish';
        } else {
            echo 'false';
...
<script type="text/javascript">
    ...
    var videoScript = $jConflict('#videoScript').val();
    ...
    $jConflict(document).ready(function () {
        ...
        $jConflict('#videoScript').focus(
            function () {
                if ($jConflict(this).val() == videoScript)
                    $jConflict(this).val("");
            }
        );
        $jConflict('#videoScript').blur(
            function () {
                if ($jConflict(this).val() == "")
                    $jConflict(this).val(videoScript);
            }
        );
        $jConflict("#videoScript").focus();

        $jConflict("#video-gallery-post").submit(function () {
            var videoScript = this.videoScript.value;
            if (videoScript == 'Video Script' || videoScript == '') {
                alert('กรุณากรอก Video Script');
                this.videoScript.focus();
                return false;
            }
            $jConflict.ajax({
                type:"POST",
                url:urlPost,
                data:$jConflict("#video-gallery-post").serialize(),
        ...
        $jConflict(".image").click(function () {
            ...
            $jConflict.ajax({
                ...
                success:function (msg) {
                    if (msg != "false") {
                        $jConflict("#tr-videoScrip").html(msg);
                        $jConflict("#videoScript").focus();
                    } else {
                        alert(msg)
                    }
        ...
        $jConflict(".deleteImage").click(function () {
            var id = this.id.split('_');
            id = id[1];
            if (!confirm('คุณต้องการลบ Video ID: ' + id + ' ใช่หรือไม่')) {
                return false;
            }
            $jConflict.ajax({
                type:"POST",
...
<?php
$listVideo = $conDB->loadListVideo();
if (!empty($listVideo)) {
    ?>
<div id="container">
    <ul id="items">
        <?php
        foreach ($listVideo as $key => $value) { //getIDImageFromUrl($value->path);
            ?>
            <li style="display: list-item;">
                <div class="imgHover" id="imgIDHover_<?php echo $value->id; ?>">
                    <p class="image" id="id-video_<?php echo $value->id; ?>">
                        <img width="100" height="60" src="<?php echo $conDB->getVideoID($value->script); ?>" alt=""/>
                    </p>
...
<h3>Upload วีดีโอ</h3>
<form action="" id="video-gallery-post" method="post">

    <table class="wp-list-table widefat" cellspacing="0">
        <thead>
        <tr>
            <th scope="col" colspan="2" class="manage-column column-name" style=""><strong><font color="#F9651B">Video
                Script</font></strong></th>
        </tr>
        </thead>
        <tbody id="the-list" class="list:user">

        <tr class="alternate" id="tr-videoScrip">
            <th scope="row" class="check-column">
                <td>
                    <input type="hidden" id="typePost" name="typePost" value="add"/>
                    <textarea rows="5" cols="40" id="videoScript" name="videoScript" placeholder="Video Script"
                              style="margin: 1px; width: 692px; height: 89px;">Video Script</textarea>
                    <br>
                    <input type="submit" name="submit" value="Save" class="button-primary"/></td>
            </th>
        </tr>
        </tbody>
    </table>

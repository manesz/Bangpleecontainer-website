...
require_once ("class/ClassPriceTable.php");
$conDB = new PriceTable($wpdb);
$checkPost = @$_POST['typePost'];
switch ($checkPost) {
    case "add":
        $imageID = $conDB->addPriceTable($_POST);
        if ($imageID != false) {
            echo $imageID;
        } else {
            echo 'false';
        ...
        break;
    case "delete":
        $result = $conDB->deletePriceTable(@$_POST['priceTableID'], 0);
        if ($result != false) {
            echo 'finish';
        } else {
            echo 'false';
        ...
        break;
    case "update":
        $result = $conDB->updatePriceTable($_POST);
        if ($result != false) {
            echo 'finish';
        } else {
            echo 'false';
        ...
        break;
    case "get":
        $result = $conDB->loadListPriceTable(@$_POST['priceTableID']);
        if ($result != false) {
            ?>
        <input type="hidden" id="typePost" name="typePost" value="update"/>
        <input type="hidden" id="priceTableID" name="priceTableID" value="<?php echo $result[0]->p_id; ?>"/>
        <input type="hidden" id="imageID" name="imageID" value="<?php echo $result[0]->i_id; ?>"/>
        <table class="wp-list-table widefat" cellspacing="0">
            <tr>
                <td width="150"><label for="priceTableTitle"><strong><font
                    color="#F9651B">Title</font></strong></label></td>
                <td><input id="priceTableTitle" type="text" name="priceTableTitle" size="100" placeholder="Title"
                           title="Title" value="<?php echo $result[0]->title; ?>"/></td>
            </tr>
            <tr>
                <td><label for="priceTablePrice"><strong><font
                    color="#F9651B">Price</font></strong></td>
                <td><input id="priceTablePrice" type="text" name="priceTablePrice" size="100" placeholder="Price"
                           title="Price" value="<?php echo $result[0]->price; ?>"/></label></td>
            </tr>
            <tr>
                <td><strong><font
                    color="#F9651B">Value</font></strong></td>
                <td id="tdValuePriceTable">
                    <?php
                    $listMapPriceTable = $conDB->getListMapPriceTable($result[0]->p_id);
                    foreach ($listMapPriceTable as $key => $value) {
                        if ($key == 0) {
                            ?>
                            <input id="priceTableValue_1" type="text" name="priceTableValue[]" size="70"
                                   placeholder="Value1"
                                   title="Value1" value="<?php echo $value->value;?>"/>&nbsp;
                            <img class="addValue" onclick="clickAdd()"
                                 src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-add.png"
                                 title="เพิ่ม Value"/>
                            <?php } else { ?>
                            <div class="valueHover" id="divValueDelete_<?php echo $key + 1;?>">
                                <input id="priceTableValue_<?php echo $key + 1;?>" value="<?php echo $value->value;?>"
                                       type="text" name="priceTableValue[]" size="70"
                                       placeholder="Value<?php echo $key + 1;?>" title="Value<?php echo $key + 1;?>"/>
                                &nbsp;<img class="deleteValue" id="imgValueDelete_<?php echo $key + 1;?>"
                                           onclick="clickDelete(this)"
                                           src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-delete.png"
                                           title="ลบ Value นี้ออก"/></div>
                            <?php
                        }
                    }?>
            ...
            <tr>
                <td colspan="2">
                    <input id="pathImg" type="text" name="pathImg" size="100" placeholder="Path Image"
                           title="Path Image" value="<?php echo $result[0]->path;?>"/>
                    <input id="uploadImageButton" type="button" class="button" value="Upload Image"
                           onclick="imageUploader('#pathImg');"/><br>
                    <input type="button" value="New" onclick="document.location.reload(true)" class="button-primary"/>
                    <input type="submit" name="gallery" value="Update" class="button-primary"/>
                </td>
            </tr>
        </table>
        <script>
            arrayValue = [
                <?php foreach ($listMapPriceTable as $key => $value) { ?>
                    [ <?php echo $key + 1;?>, '<?php echo $value->value;?>'],
                    <?php }?>
            ];
        </script>
        <?php
...
<link href="<?php bloginfo('template_directory');?>/theme-option-controller/css/easypaginate-price-table.css" rel="stylesheet"
      type="text/css"/>
...
<script type="text/javascript">
    ...
    $jConflict(document).ready(function () {
        ...
        $jConflict("#priceTableTitle").focus();
        $jConflict("#price-table").submit(function () {
            $jConflict.ajax({
                type:"POST",
                url:urlPost,
                data:$jConflict("#price-table").serialize(), //$("#formEditZone").serialize()
        ...
        $jConflict(".deleteImage").click(function () {
            var id = this.id.split('_');
            id = id[1];
            if (!confirm('คุณต้องการลบภาพ ID: ' +id+ ' ใช่หรือไม่')){
                return false;
            }
            $jConflict.ajax({
                type:"POST",
        ...
        $jConflict(".image").click(function () {
            var id = this.id.split('_');
            id = id[1];
            $jConflict.ajax({
                type:"POST",
        ...
        var idHover;
        $jConflict(".imgHover").mouseover(function () {
            var id = this.id.split("_");
            idHover = id[1];
            showImg("#btnDelete_" + idHover);
        }).mouseout(function () {
                hideImg("#btnDelete_" + idHover);
            });

        var idHover2;
        $jConflict('.valueHover').live('mouseover',function () {
            var id = this.id.split("_");
            idHover2 = id[1];
            showImg("#imgValueDelete_" + idHover2);
        }).live('mouseout', function () {
                hideImg("#imgValueDelete_" + idHover2);
            });
    });
    var countValueSave = 1;

    function readValueFromInput() {
        if (arrayValue.length > 0) {
            for (i = 0; i < arrayValue.length; i++) {
                var id = i + 1;
                id = "#priceTableValue_" + id;
                arrayValue[i][1] = $jConflict(id).val();
            }
        }
    }

    function refreshArrayValue(id) {
        var count = 0;
        var newStr = '';
        var newArray = [];
        for (i = 0; i < arrayValue.length; i++) {
            if (i + 1 != (id)) {
                ...
                count++;
                newStr += getStrHtml(count, arrayValue[i][1]);
            }
        }
        arrayValue = newArray;
        $jConflict("#tdValuePriceTable").html(newStr);
    }

    function getStrHtml(id, value) {
        if (id == 1) {
            var strHtml = '<input id="priceTableValue_1" type="text" name="priceTableValue[]" size="70" placeholder="Value1"' +
                'title="Value1" value="' + value + '"/>' +
                '&nbsp;<img class="addValue"' + ' onclick="clickAdd()"' +
                'src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-add.png"' +
                'title="เพิ่ม Value"/>';
        } else {
            var strHtml =
                '<div class="valueHover" id="divValueDelete_' + id + '"><input id="priceTableValue_' + id + '" value="' + value +
                    '" type="text" name="priceTableValue[]" size="70" placeholder="Value' + id + '" title="Value' + id + '"/>' +
                    '&nbsp;<img class="deleteValue" id="imgValueDelete_' + id + '" onclick="clickDelete(this)"' +
                    ' src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-delete.png" title="ลบ Value นี้ออก"/></div>';
        }
        return strHtml;
    }
    ...
    function clickAdd() {
        arrayValue.push([ 0, '']);
        var strHtml = getStrHtml(arrayValue.length, '');
        $jConflict("#tdValuePriceTable").append(strHtml);
    }
    ...
    function clickDelete(event) {
        ...
        refreshArrayValue(id);
    }
</script>
<?php
$listPriceTable = $conDB->loadListPriceTable();
if (!empty($listPriceTable)) {
    ?>
<div id="container">
    <ul id="items">
        <?php
        foreach ($listPriceTable as $key => $value) {
            ?>
            <li style="display: list-item;">
                <div class="imgHover" id="imgIDHover_<?php echo $value->p_id; ?>">
                    <p class="image" id="imgShow_<?php echo $value->p_id; ?>">
                        <img width="100" height="200"
                             src="<?php echo $value->path; ?>" alt=""
                             title="<?php echo $value->title; ?>">
                    </p>
                    <!--<h3>Template --><?php //echo $value->g_id; ?><!--</h3>-->
                    <p class="info"></p>

                    <div class="btnDeletePriceTable" id="btnDelete_<?php echo $value->p_id; ?>">
                        <img class="deleteImage" id="deleteImage_<?php echo $value->p_id; ?>"
                             src="<?php bloginfo('template_directory');?>/theme-option-controller/images/deleteImg.png"
                             title="ลบภาพนี้ออก"/>
                    </div>
                </div>
            </li>
...
<!--<h3>Upload Slide</h3>-->
<form action="" id="price-table" method="post">
    <div id="div-inner">
        <input type="hidden" id="typePost" name="typePost" value="add"/>
        <table class="wp-list-table widefat" cellspacing="0">
            <tr>
                <td width="150"><label for="priceTableTitle"><strong><font
                    color="#F9651B">Title</font></strong></label></td>
            ...
            <tr>
                <td><label for="priceTablePrice"><strong><font
                    color="#F9651B">Price</font></strong></td>
                <td><input id="priceTablePrice" type="text" name="priceTablePrice" size="100" placeholder="Price"
                           title="Price"/></label></td>
            </tr>
            <tr>
                <td><strong><font
                    color="#F9651B">Value</font></strong></td>
                <td id="tdValuePriceTable">
                    <input id="priceTableValue_1" type="text" name="priceTableValue[]" size="70" placeholder="Value1"
                           title="Value1"/>
                    <img class="addValue" onclick="clickAdd()"
                         src="<?php bloginfo('template_directory');?>/theme-option-controller/images/btn-add.png"
                         title="เพิ่ม Value"/>
